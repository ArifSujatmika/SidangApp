<x-layouts::app :title="__('Tambah Jadwal')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        ...
        <form method="POST" action="{{ route('admin.schedules.store') }}" class="space-y-4 rounded-xl border border-neutral-200 p-4 dark:border-neutral-700">
            @csrf
            <div>
            ...
            >
                <div class="mb-2 flex flex-wrap items-center justify-between gap-2">
                    <label for="participant-search-create" class="text-sm font-medium">Mahasiswa Peserta</label>
                    <div class="flex items-center gap-3 text-xs">
                        <span class="rounded-full bg-teal-50 px-2.5 py-1 font-medium text-teal-700 dark:bg-teal-950/50 dark:text-teal-300" x-text="`${selected.length} dipilih`"></span>
                        <button type="button" class="font-medium text-teal-600 hover:underline dark:text-teal-400" @click="toggleAll()" x-text="selected.length === options.length && options.length ? 'Hapus semua' : 'Pilih semua'"></button>
                    </div>
                ...
                <div class="relative mb-2">
                    <input x-model="query" id="participant-search-create" type="search" placeholder="Cari nama atau NIM..." class="w-full rounded-lg border border-neutral-200 bg-white px-3 py-2 text-sm outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20 dark:border-neutral-700 dark:bg-neutral-900">
                </div>
